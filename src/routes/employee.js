...
const LookupSchema = z.object({
  ...
  dateOfBirth: z.string().min(1).max(32) // yyyy-mm-dd
});
...
async function tryFindInTable(pool, table, employeeId, dob) {
  const result = await pool
    ...
    .query(
      `SELECT TOP 1 EMP_NO, EMP_NM, BIRTH_DT, JOIN_DT, Type FROM dbo.${table} WHERE EMP_NO=@EmpNo AND BIRTH_DT=@Dob`
    );
...
async function findQuarterEmpClass(pool, { classNo, type }) {
  // Class1 has Sr/Jr split by priority; others match by name, fallback to priority.
  if (classNo === 1) {
    ...
    const result = await pool
      ...
      .query(
        "SELECT TOP 1 Class_PRIORITY, class_name, [Class] FROM dbo.Quarter_Emp_Class WHERE Class_PRIORITY=@Priority"
      );
  ...
  const byName = await pool
    ...
    .query(
      "SELECT TOP 1 Class_PRIORITY, class_name, [Class] FROM dbo.Quarter_Emp_Class WHERE LOWER(class_name) LIKE LOWER(@LikeName) ORDER BY Class_PRIORITY ASC"
    );
  if (byName.recordset?.[0]) return byName.recordset[0];

  const fallbackPriority = classNo + 1; // assumes Class1 Jr = 2, Class2 = 3, Class3 = 4, Class4 = 5
...
router.post("/lookup", async (req, res) => {
  ...
  const empClass = classNo ? await findQuarterEmpClass(pool, { classNo, type: found.row?.Type }) : null;

  return res.json({
    employeeId: found.row.EMP_NO,
    employeeName: found.row.EMP_NM,
    dateOfBirth: new Date(found.row.BIRTH_DT).toISOString().slice(0, 10),
    dateOfJoining: found.row.JOIN_DT ? new Date(found.row.JOIN_DT).toISOString().slice(0, 10) : "",
...
router.get("/me", requireAuth, async (req, res) => {
  ...
  const details = await pool
    ...
    .query("SELECT TOP 1 EmployeeId, DateOfBirth, EmployeeName FROM dbo.UserDetails WHERE UserId=@UserId");

  const row = details.recordset?.[0];
  if (!row) return res.status(404).json({ error: "User details not found" });

  const employeeId = String(row.EmployeeId || "").trim();
  const dob = row.DateOfBirth ? new Date(row.DateOfBirth).toISOString().slice(0, 10) : "";
  if (!employeeId || !dob) return res.status(400).json({ error: "Missing employee details" });
  ...
  for (const table of tables) {
    ...
    const r = await pool
      ...
      .input("EmpNo", sql.NVarChar(50), employeeId)
      .input("Dob", sql.Date, new Date(dob))
      .query(`SELECT TOP 1 EMP_NO, EMP_NM, BIRTH_DT, JOIN_DT, Type, CAST FROM dbo.${table} WHERE EMP_NO=@EmpNo AND BIRTH_DT=@Dob`);
    if (r.recordset?.[0]) {
  ...
  const casteOfEmployee = castRaw === "SC" ? "SC" : castRaw === "ST" ? "ST" : "GENERAL";
